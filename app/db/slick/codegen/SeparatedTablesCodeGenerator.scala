...
class SeparatedTablesCodeGenerator(metamodel: Model, outputDir: String, pkg: String,
                                   ...
                                   sharedTypesMapping: Map[String, Map[String, String]]) extends SourceCodeGenerator(metamodel) with TaggedExt {
  ...
  class TableExt(tbl: slick.model.Table) extends super.TableDef(tbl) {
    ...
    class ColumnExtended(model: slick.model.Column) extends super.ColumnDef(model) with TaggedModelColumn {
      ...
      override def rawType: String =
        (fkType orElse taggedModel.map(_.typeName) orElse sharedTypeModel) | customType(model)
      ...
      private def customType(mdl: slick.model.Column): String =
        mdl.tpe match {
          ...
          case "String" =>
            mdl.options
              .collectFirst { case x: ColumnOption.SqlType if x.typeName == "jsonb" => "play.api.libs.json.JsValue" }
              .getOrElse("String")
...
object SeparatedTablesCodeGenerator {
  ...
  def main(args: Array[String]): Unit =
    args.toList match {
      case configPath :: outputDir :: Nil =>
        ...
        run(slickDriver, jdbcDriver, url, outputDir, pkg, user.some, password.some, includedTables, ignoredTables, taggedTypesMapping | Map.empty, sharedTypes | Map.empty, sharedTypesMapping | Map.empty)
      case _ =>
  ...
  def run(slickDriver: String,
          ...
          taggedTypesMapping: Map[String, Seq[String]],
          sharedTypes: Map[String, String],
          sharedTypesMapping: Map[String, Map[String, String]]): Unit = {
    ...
    try {
      ...
      new SeparatedTablesCodeGenerator(m, outputDir, pkg, taggedTypesMapping, sharedTypes, sharedTypesMapping).writeToFile(slickDriver, outputDir, pkg)
    } finally db.close
  }
